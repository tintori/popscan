...
plotTraces <- function(sd.table, exp.design, 
    ...
    custom_color = NULL
    if(!is.factor(exp.design$colorer)){exp.design$colorer = as.factor(exp.design$colorer)}
    tmp.color.ln = length( na.omit(levels(exp.design$colorer) ))
    if(tmp.color.ln==1){ 
    ...
    if(tmp.color.ln %in% c(2:14)) {
        custom_color = scale_color_manual(values = pinkgreen8[sort(c(color_adding_order[1:tmp.color.ln]))],
                                          limits=na.omit(levels(exp.design$colorer)), 
                                          drop = FALSE)
    }
